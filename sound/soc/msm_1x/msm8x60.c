			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (IS_ERR(rx_dev_info)) {
		pr_err("pass invalid dev_id\n");
		rc = PTR_ERR(rx_dev_info);
		return rc;
	}

	if (!(rx_dev_info->capability & SNDDEV_CAP_RX)) {
		pr_err("First Dev is supposed to be RX\n");
		return -EFAULT;
	}
	...
	if (!(tx_dev_info->capability & SNDDEV_CAP_TX)) {
		pr_err("Second Dev is supposed to be Tx\n");
		return -EFAULT;
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (set) {
		if (!dev_info->opened) {
			...
			pr_info("device freq =%d\n", set_freq);
			rc = dev_info->dev_ops.set_freq(dev_info, set_freq);
			if (rc < 0) {
				pr_err("device freq failed!\n");
				return rc;
			}
			...
			if (rc < 0) {
				pr_err("Enabling %s failed", dev_info->name);
				return rc;
			}
			dev_info->opened = 1;
		...
		if (dev_info->opened) {
			...
			if (rc < 0) {
				pr_err("Snd device failed close!\n");
				return rc;
			} else {
				dev_info->opened = 0;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rc < 0) {
		pr_err("device could not be assigned!\n");
		return -EFAULT;
	}
