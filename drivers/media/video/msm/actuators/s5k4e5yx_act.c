...
#define	S5K4E5YX_TOTAL_STEPS_NEAR_TO_FAR			256
#else
...
static int32_t s5k4e5yx_poweron_af(void)
{
	...
	pr_info("[CAM] %s enable AF actuator, gpio = %d\n", __func__,
	...
	/* TODO */
	if (rc != -EBUSY && rc != 0) {
		pr_warning("[CAM] %s: AF Power on gpio_request failed %d\n", __func__, rc);
	}
	...
	if (!rc) {
		...
		pr_err("[CAM] %s: AF Power on gpio_request failed %d\n", __func__, rc);
	}
...
{
	pr_info("[CAM] %s disable AF actuator, gpio = %d\n", __func__,
			s5k4e5yx_msm_actuator_info->vcm_pwd);
	...
	rc = gpio_request(s5k4e5yx_msm_actuator_info->vcm_pwd, "s5k4e5yx");
	if (!rc)
		gpio_direction_output(s5k4e5yx_msm_actuator_info->vcm_pwd, 0);
	else
		pr_err("[CAM] %s: AF Power off gpio_request failed %d\n", __func__, rc);
...
#ifdef USE_RAWCHIP_AF
			if (s5k4e5yx_msm_actuator_info->use_rawchip_af)
...
int32_t s5k4e5yx_msm_actuator_move_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
	int dir,
	int32_t num_steps)
{
...
static struct msm_actuator_ctrl_t s5k4e5yx_act_t = {
	...
	.set_info = {
		.total_steps = S5K4E5YX_TOTAL_STEPS_NEAR_TO_FAR,
		.gross_steps = 3,	/*[TBD]*/
		.fine_steps = 1,	/*[TBD]*/
	},
