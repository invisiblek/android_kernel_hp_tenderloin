...
static int msm8930_rawchip_vreg_on(void)
{
	...
	if (rc < 0) {
		pr_err("[CAM] rawchip on (\"gpio %d\", 1.8V) FAILED", CAM_PIN_GPIO_V_CAM_D1V8_EN);
	...
	if (rc < 0) {
		pr_err("[CAM] rawchip on (\"gpio %d\", 1.2V) FAILED", CAM_PIN_GPIO_V_RAW_1V2_EN);
...
static int msm8930_rawchip_vreg_off(void)
{
	...
	if (rc < 0)
		pr_err("[CAM] rawchip off (\"gpio %d\", 1.2V) FAILED", CAM_PIN_GPIO_V_RAW_1V2_EN);
	else {
	...
	if (rc < 0)
		pr_err("[CAM] rawchip off (\"gpio %d\", 1.8V) FAILED", CAM_PIN_GPIO_V_CAM_D1V8_EN);
	else {
		gpio_direction_output(CAM_PIN_GPIO_V_CAM_D1V8_EN, 0);
		gpio_free(CAM_PIN_GPIO_V_CAM_D1V8_EN);
	}
