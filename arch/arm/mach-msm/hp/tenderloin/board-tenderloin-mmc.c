/* linux/arch/arm/mach-msm/board-tenderloin-mmc.c
 *
...
static uint32_t wifi_on_gpio_table[] = {
	...
	GPIO_CFG(TENDERLOIN_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA), /* WLAN IRQ */
};
...
static uint32_t wifi_off_gpio_table[] = {
	...
	GPIO_CFG(TENDERLOIN_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* WLAN IRQ */
};
 ...
 */
static struct embedded_sdio_data tenderloin_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
tenderloin_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int tenderloin_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int tenderloin_wifi_status(struct device *dev)
{
	return tenderloin_wifi_cd;
}

static struct mmc_platform_data tenderloin_wifi_data = {
        .ocr_mask               = MMC_VDD_28_29,
        .status                 = tenderloin_wifi_status,
        .register_status_notify = tenderloin_wifi_status_register,
        .embedded_sdio          = &tenderloin_wifi_emb_data,
...
int tenderloin_wifi_set_carddetect(int val)
{
	printk(KERN_INFO "%s: %d\n", __func__, val);
	tenderloin_wifi_cd = val;
	if (wifi_status_cb)
...
EXPORT_SYMBOL(tenderloin_wifi_set_carddetect);
...
int tenderloin_wifi_power(int on)
{
	...
	gpio_set_value(TENDERLOIN_GPIO_WIFI_SHUTDOWN_N, on); /* WIFI_SHUTDOWN */

	mdelay(120);
	return 0;
}
EXPORT_SYMBOL(tenderloin_wifi_power);

int tenderloin_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
	return 0;
}

int __init tenderloin_init_mmc()
{
	uint32_t id;
	wifi_status_cb = NULL;

	printk(KERN_INFO "tenderloin: %s\n", __func__);

	/* initial WIFI_SHUTDOWN# */
	id = GPIO_CFG(TENDERLOIN_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
	gpio_tlmm_config(id, 0);
	gpio_set_value(TENDERLOIN_GPIO_WIFI_SHUTDOWN_N, 0);

	msm_add_sdcc(4, &tenderloin_wifi_data);
