...
void send_cal(int port_id, struct acdb_cal_block *aud_cal)
{
	...
	if (!aud_cal || aud_cal->cal_size == 0) {
		pr_info("%s: No calibration data to send!\n", __func__);
		goto done;
	}
	...
	pr_info("%s: Sending SET_PARAMS payload = 0x%x, size = %d\n",
...
void send_adm_cal(int port_id, int path)
{
	...
	pr_info("%s: Sending audproc cal, acdb_path %d\n",
		    __func__, acdb_path);
	get_audproc_cal(acdb_path, &aud_cal);
	send_cal(port_id, &aud_cal);

	pr_info("%s: Sending audvol cal, acdb_path %d\n",
		    __func__, acdb_path);
			...
			unsigned int *port_id, int copp_id)
{
	...
	for (i = 0; i < num_copps; i++) {
		...
		if (tmp < 0 || tmp >= AFE_MAX_PORTS) {
			pr_err("afe_get_port_index return invalid %d\n", tmp);
			ret = -EINVAL;
			goto fail_cmd;
		}
